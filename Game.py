...
def add_disc(DiscFactory):
	...
	input("New disc is %s" % _new_disc.name)
...
def test(PlayersDiscamals, WinnerList):
	...
	input("%s wins the test." % format_team(get_winner(_d1, _d2, WinnerList)))
...
def tournament(AvailableDiscamals, Players, WinnerList, TeamSize):
	...
	input("%s won the tournament with %s." % (_winner.name, format_team(_winner.currentTeam)))
...
if __name__ == '__main__':
	...
	input("Take today to try them out.")
